...
namespace HydrogenCG
{
	...
	*/
	struct Camera
	{
		...
		*/
		inline Ray generateRay(const vec2& uv) const
		{
			return Ray(o, normalize(uv.x * e0 + uv.y * e1 + e2));
		}

		// convenience/brevity
		inline Ray operator()(const vec2& uv) const
		{
			return generateRay(uv);
		}
	};

	// free function variant, mirrors the shadertoy code
	Ray generateRay(const Camera& cam, const vec2& uv)
	{
		return cam.generateRay(uv);
	}
}